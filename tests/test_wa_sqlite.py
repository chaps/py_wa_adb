...
def test_get_messages_query():
    assert WA_SQLITE.get_messages() == f"SELECT * FROM {WA_SQLITE.MESSAGES_TABLE_NAME};"


def test_get_messages_query_filter_jid():
    assert WA_SQLITE.get_messages(key_remote_jid=TEST_JID) == \
    f"SELECT * FROM {WA_SQLITE.MESSAGES_TABLE_NAME} WHERE{WA_SQLITE.FILTER_FROM_JID.format(key_remote_jid=TEST_JID)};" 
...
def test_get_messages_query_filter_from_date():
    ...
    f"SELECT * FROM {WA_SQLITE.MESSAGES_TABLE_NAME} WHERE{WA_SQLITE.FILTER_FROM_DATE.format(filter_from_date_epoch=t)};" 
...
def test_get_messages_query_filter_to_date():
    ...
    f"SELECT * FROM {WA_SQLITE.MESSAGES_TABLE_NAME} WHERE{WA_SQLITE.FILTER_TO_DATE.format(filter_to_date_epoch=t)};" 
...
def test_get_messages_query_filter_jid_and_from_date():
    ...
    f"SELECT * FROM {WA_SQLITE.MESSAGES_TABLE_NAME} WHERE{WA_SQLITE.FILTER_FROM_JID.format(key_remote_jid=TEST_JID)}AND{WA_SQLITE.FILTER_FROM_DATE.format(filter_from_date_epoch=t)};" 
